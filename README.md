...
2. **Contiguous Out-of-Time Boundary:** Data is partitioned chronologically across reporting months (default: 12-month holdout). This strictly mirrors production deployment where future performance months are unobserved.
3. **The Honest Evaluation Audit:** Because panel datasets can feature the same borrower across multiple historical months, flexible models can inflate performance through borrower identity memorization. LPIE runs a 5-fold **loan-disjoint holdout ablation** alongside the time split, proving that the model captures true cross-borrower behavioral signal (16k demo: disjoint default AUC 0.6527 [0.598–0.711] vs 0.9595 time-split; 210k pack: 0.6935 [0.671–0.716] vs 0.8784) rather than mere memorization.
